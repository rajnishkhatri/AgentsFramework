...
`AL-13` **End on resolution.** A self-contained lesson SHOULD end on `rule`. Permitted exception: `refresher` (ASC) ends on `tension` as a parting caution, because the rule already led. Never end on `tension` when the rule has not yet appeared. **[E1, block layer — refines for the composer]** In the inductive `newSkill` the `rule` sits mid-recipe and *application* blocks (`workedExample`, `completionTry`) follow it. The block-layer guard: the **main zone ends on the deepest available resolution** — the applied **win** (`completionTry`) when application blocks are present, else the `rule`. This does not violate the beat law (AL-13 forbids ending on `tension` *before the rule appears*; here the rule appears, then is applied). The **rail** (`accuracyStat`/`dueChecklist`/`coachEntry`) is ambient and exempt.
...
### 5.1 Context selection — [PROD, E1] the 3-context projection

E1 binds the **3-context block composer** (`newSkill | returning | refresher`), not the 6-mode engine, because the outer loop emits only `mastery`, `missTag`, `due` — not `feelsProblem`, `prefersWorkedExamples`, `prefersVisual`, or `justMissed`. `selectLessonContext` is the faithful 6→3 projection of `selectMode`: `struggling`/`diagnostic` (and `worked`-when-due) collapse to `returning`; `worked`/`annotated`-when-not-due are indistinguishable from teaching without the `prefers*` signals, so they fold to `newSkill`.

```
selectLessonContext({ firstExposure, masteryPct, dueMisses, requested }):
  if requested                           → requested     // AL-17 analog: explicit pick wins
  if firstExposure or masteryPct == null → 'newSkill'    // AL-16 analog: safe default
  if masteryPct >= 80 and dueMisses == 0 → 'refresher'   // the one transferable numeric anchor
  if dueMisses > 0                       → 'returning'    // review debt to clear — tagged or not
  else                                   → 'newSkill'     // learning; nothing due — keep teaching
```

`AL-16b` The `newSkill → returning` flip is `dueMisses > 0` — **no mastery threshold gates it**; mastery gates only `refresher` (≥ 80 & `due == 0`). `AL-17b` Due-ness selects the context; a misconception **tag never does** — a tag on a non-due miss does not force `returning` (the scheduler owns re-surfacing); within `returning` the newest due miss's tag *populates* the callout, and an untagged due miss still routes to `returning` (callout self-hides). `AL-16c` `newSkill` also fires for "learning, `< 80`, nothing due," so its data-driven blocks (`accuracyStat`) gate on **data availability, not on the context label** (omit when no session history exists). Full rationale + D1–D8/I1: `Adaptive-Lesson-Decisions.md`.
