...
## Block catalog (v1 — 9 tags · E1 adds 3 beat-promotions → 12)
...
| `ground` | main | neutral | title, body | inductive opener — familiar footing at first exposure; the lead block |
| `question` | main | accent | prompt | the one-line framing question the tension forces (inductive `newSkill`) |
| `pitfall` | main | warning | label, body | first-exposure tension — the skill's **structural** trap, no miss data |

> **E1 addition** — `ground`, `question`, `pitfall` promote the Adaptive beats (ground / question / tension) into the catalog so the composer can render the **inductive** `newSkill`. `pitfall` and `misconceptionCallout` are two treatments of the one **tension / warning** beat role, selected by context: `pitfall` (generic, structural) at first exposure, `misconceptionCallout` (miss-specific) on return.
...
|---|---|---|
| **New skill** *(inductive)* | discover-first, apply to a win | ground → pitfall → question → selfExplainPrompt → rule → workedExample → completionTry · rail: accuracyStat *(omit when no history)* |
...
> **E1 reconciliation** — `newSkill` now follows the protocol's `new` = SCQA **inductive** arc (ground → tension → question → rule) rather than the earlier rule-first (deductive) recipe, which was the `refresher` = ASC ordering wearing the `new` label. `completionTry` closes on the applied win (AL-13 block layer). Selection contract: `selectLessonContext` (Protocol §5.1). Full rationale: `Adaptive-Lesson-Decisions.md` (A1–A3).
