...
SEED_AGENTS: list[AgentFacts] = [
    AgentFacts(
        ...
        policies=[
            Policy(
                name="never-rm-rf", description="Reject destructive shell commands."
            ),
            Policy(
                name="repo-sandbox", description="File I/O is sandboxed to repo root."
            ),
        ],
    ),
    AgentFacts(
...
def generate_workflows(
    ...
    for i in range(count):
        ...
        for guardrail in rng.sample(
            GUARDRAIL_TYPES, k=rng.randint(1, len(GUARDRAIL_TYPES))
        ):
            recorder.record(
                TraceEvent(
                    event_id=str(uuid.UUID(int=rng.getrandbits(128))),
            ...
            recorder.record(
                TraceEvent(
                    ...
                    details={
                        "agent_id": agent_id,
                        "model": model,
                        "planned_action": "tool_then_model"
                        if step % 2 == 0
                        else "model_only",
                    },
                )
