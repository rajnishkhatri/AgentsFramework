...
def depth_rows() -> list[dict]:
    return [
        ...
        _row(
            ...
            want_depth="L0",
            note="TRAP: long path, single create -> stays L0 (not long-task-floor)",
        ),
        _row(
            "depth-l0-trap-2",
            "long-path-l0",
            ...
            want_depth="L0",
            note="TRAP: long path single write -> L0",
        ),
        # L1 — lone strong-intent verbs
        _row(
            "depth-l1-1",
            "lone-marker:investigate",
            "depth",
            "Investigate the latency regression.",
            want_depth="L1",
            note="strong-intent verb floor",
        ),
        _row(
            "depth-l1-2",
            "lone-marker:audit",
        ...
        _row(
            ...
            want_depth="L1",
            note=">=25 words, no stacked markers -> L1",
        ),
        ...
        _row(
            ...
            want_depth="L2",
            note="3 stacked signals -> high-complexity",
        ),
        ...
        _row(
            ...
            want_depth="L2",
            note="incident markers + length -> L2",
        ),
        ...
        _row(
            ...
            want_depth="L2",
            note="TRAP: multi-marker prose, intended L2 (known L2->L1 miss)",
        ),
        _row(
            "depth-l2-trap-2",
            "l2-under-promote",
            ...
            want_depth="L2",
            note="TRAP: redesign+migrate+refactor prose -> intended L2",
        ),
        _row(
            "depth-l2-trap-3",
            "l2-under-promote",
            ...
            want_depth="L2",
            note="TRAP: investigate+design+refactor prose -> intended L2",
        ),
        _row(
            "depth-l2-trap-4",
            "l2-under-promote",
            ...
            want_depth="L2",
            note="TRAP: architecture+design+migrate prose -> intended L2",
        ),
    ]
...
def count_scope_rows() -> list[dict]:
    pairs = [
        (
            "Compare Kafka and RabbitMQ for our event bus, (1) measure throughput "
            "(2) test ordering (3) assess cost, and then recommend one.",
            "L2",
        ),
    ...
    for i, (prompt, fresh_depth) in enumerate(pairs, 1):
        ...
        rows.append(
            _row(
                f"count-posttool-{i}",
                "count-scope:post-tool",
                "depth",
                prompt,
                task_tool_results_count=2,
                want_depth="L0",
...
def branch_rows() -> list[dict]:
    return [
        ...
        _row(
            ...
            want_branch_count=3,
            note="3 bullet markers stripped",
        ),
        # inline enumeration
        ...
        _row(
            ...
            want_branch_count=4,
            note="lead-in clause + (1)(2)(3) -> 4 branches",
        ),
        ...
        _row(
            ...
            want_branch_count=3,
            note="pure (1)(2)(3) enumeration, no lead-in -> 3",
        ),
        ...
        _row(
            ...
            want_branch_count=3,
            note="X, Y, and Z imperative -> 3 (comma-then-and)",
        ),
        # ", then" sequencing
        _row(
            "branch-then-1",
            ...
            want_branch_count=2,
            note=", then -> 2 imperative clauses",
        ),
        ...
        _row(
            ...
            want_branch_count=1,
            note="single action -> 1 branch",
        ),
        # TRAP — path-safety: periods inside paths/versions must NOT split.
        _row(
            "branch-trap-path-1",
            "split-trap:path-safe",
            "branches",
            "Open /workspace/f3.txt and read it.",
            want_branch_count=1,
            note="TRAP: /workspace/f3.txt must not sentence-split",
        ),
        _row(
            "branch-trap-version-1",
            "split-trap:path-safe",
            "branches",
            "Pin the dependency to v1.2.3 in the lockfile.",
            want_branch_count=1,
            note="TRAP: v1.2.3 must not sentence-split",
        ),
        # TRAP — noun-phrase "and" must NOT split (no leading comma, not imperative)
        _row(
            "branch-trap-nounphrase-1",
            "split-trap:noun-phrase",
            "branches",
            "Summarize the trade-offs and risks of the new design.",
            want_branch_count=1,
            note="TRAP: 'trade-offs and risks' is a noun phrase, 1 branch",
        ),
        _row(
            "branch-trap-nounphrase-2",
            "split-trap:noun-phrase",
            "branches",
            "Document the costs and benefits of the migration.",
            want_branch_count=1,
            note="TRAP: 'costs and benefits' noun phrase -> 1 branch",
        ),
    ]
...
def condition_rows() -> list[dict]:
    return [
        _row(
            ...
            "conditions",
            "Restart the worker pool.",
            want_min_conditions=2,
            want_generic_tail=True,
...
def mece_rows() -> list[dict]:
    return [
        _row(
            ...
            want_mece_valid=True,
            note="contiguous ids, distinct goals, non-empty conds -> valid",
        ),
        _row(
            "mece-dupgoal-1",
            "mece:dup-goal",
            ...
            want_mece_valid=False,
            want_mece_issue="overlapping goals",
        ...
        _row(
            ...
            want_mece_valid=False,
            want_mece_issue="contiguous step_id",
        ...
        _row(
            ...
            want_mece_valid=False,
            want_mece_issue="non-empty goal",
        ...
        _row(
            ...
            want_mece_valid=False,
            want_mece_issue="success_conditions",
...
def replan_rows() -> list[dict]:
    ...
    return [
        ...
        _row(
            ...
            want_stale=True,
            note="non-empty error -> stale",
        ),
        _row(
            "replan-outcome-failed",
            "replan:outcome",
            "replan",
            base,
            last_tool_result={"outcome": "failed", "tool_name": "http"},
            want_stale=True,
            note="outcome=failed -> stale",
        ),
        _row(
            "replan-surprising",
            "replan:surprising",
        ...
        _row(
            ...
            want_stale=False,
            note="clean success -> NOT stale (continues to evaluate)",
        ),
        _row(
            "replan-clean-2",
            "replan:clean",
            ...
            want_stale=False,
            note="explicit success outcome -> NOT stale",
        ),
        _row(
            "replan-none",
            "replan:none",
            ...
            want_stale=False,
            note="no tool result -> NOT stale (nothing to invalidate)",
        ),
    ]
...
def oracle_subset() -> list[dict]:
    ...
    for i, line in enumerate(_ORACLE.read_text().splitlines(), 1):
        ...
        rows.append(
            _row(
                f"oracle-{i}",
                f"oracle:{d.get('trigger_family', 'unknown')}",
                "depth",
                d["prompt"],
                want_depth=d["want_depth"],
