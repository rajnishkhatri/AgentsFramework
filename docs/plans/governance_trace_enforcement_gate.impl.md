...
> - **Pure decision in `services/governance/carrier_gate.py`:** `decide_enforcement(gap, *, mode) -> EnforcementDecision` (never raises/does I/O — returns the action), `record_enforcement(...)` emits the loud `source: "carrier_gate_enforce"` carrier (distinct from the shadow `carrier_gate`), and `CarrierGateViolation` (raised by the *caller*, not the service — AP-4). 9 L2 tests (decision matrix rejections-first + loud-carrier contract).
> - **Orchestration acts (`_shadow_check_phase_carriers`):** after the shadow record, if mode≠off and gap non-empty, records the enforced carrier then — `raise` ⇒ `raise CarrierGateViolation` (dev/CI); `degrade` ⇒ continue (prod). The carrier is recorded BEFORE the raise so a dev failure is as auditable as a prod degrade. 4 L4 wiring tests (deterministic, in CI).
> - **`composition.py`** reads `CARRIER_GATE_ENFORCE_ENABLED` and derives the mode from `agent_env` (dev→raise, prod→degrade); 3 derivation tests. **`AGENT_ENV` already flows through composition** so no new env is needed for the split — only the one enable flag.
> - **Verification:** architecture 99✓, full carrier-gate suite 52✓ (39→+13), regression 390✓, determinism ×10, **prod-parity proven** (default mode = shadow-only, no enforce, no raise even on a real gap).
> - **AP-5 note:** the wiring shim is now ~25 logic lines (over the strict ≤15) — but every added line is *delegation* (call `decide_enforcement` → branch on `.action` → `record_enforcement`/raise); zero policy is computed in orchestration. AP-5's intent (no business logic in the node) holds; the count reflects the act-on-decision branching, not embedded logic.
> - **⚠️ DEPLOY:** flipping enforce ON in prod needs `CARRIER_GATE_ENFORCE_ENABLED` in `infra/gcp/cloud-run-backend.tf` (a Terraform `env` block mirroring `GOAL_JUDGE_ENABLED`) — Phase 2 reintroduces the "env var not in Terraform" concern Phase 1 avoided. NOT needed until the flag is actually flipped (default-OFF ships safely via a normal `backend` apply with no Terraform change).
...
| **2 — ENFORCE (BUILT 2026-06-17, ships dark)** | missing required carrier ⇒ **raise in dev / loud-degrade in prod** via the loud `carrier_gate_enforce` carrier (never silent). `carrier_gate_enforce_mode` default `"off"` (prod parity) | gate; flag `CARRIER_GATE_ENFORCE_ENABLED` default-OFF, mode derived from `AGENT_ENV` | **to FLIP ON:** Phase-1 shadow false-positive rate ≈ 0 over N runs + a live gap-catch proof + explicit approval — none met yet, so it stays OFF |
