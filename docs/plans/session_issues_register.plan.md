...
- The teaching recipes for the related trace-gap program (G1–G9, [docs/plans/trace_gap_closure.plan.md](trace_gap_closure.plan.md)) are [Recipe 9 — Proving the Guard Showed Up](../recipes/guardrails/09_rail_observability_and_determinism.md) (observable + deterministic rails, G2/G3) and [Recipe 13 — The Gate That Only Ever Says Yes](../recipes/13_negative_path_traces_and_schema_versioning.md) (negative-path traces + bundle schema versioning, G4/G7/G8, with G9 dataset coverage). The latter directly addresses the TAP-4 "gate that only accepts" risk that this register's success-only traces would otherwise leave untested.
