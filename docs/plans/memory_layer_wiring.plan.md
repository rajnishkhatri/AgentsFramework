...
**Remaining (all explicitly deferred / ask-first):**
1. **Phase-2 eval calibration** (Stage 0–7) — needs collected shadow traces before the autocapture write-back flag can flip.
2. **Live exercise of Piece C** — `tofu apply` under `infra/dev-tier/` to materialize `DATABASE_URL`, then apply the thread migration on a `--tag` no-traffic Cloud Run revision (prod untouched). Piece B is already live-validated.
3. **Commit** — user handles commits; nothing committed since `935433e` (Phase 1).
4. **Governance-trace-audit gate** on a memory-ON live trace (Verification step 4) once the loop runs with `MEMORY_ENABLED=true` on a real revision.
