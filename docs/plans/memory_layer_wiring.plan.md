---
type: plan
title: 'Add a Memory Layer to the Agent (wire the orphaned long-term memory into the react loop)'
description: 'Gates re-run today, both rings, full:'
tags: [plan]
---
...
## 📊 Implementation status — consolidated (verified 2026-06-18)

> This dashboard is the single source of truth for "where are we now." The dated narrative blocks below it are the chronological audit trail (kept, not summarized). Branch `feat/memory-layer-wiring`; last commit `bb3fb32` (Phase 2). **67 files uncommitted** (Phase 1 is committed at `935433e`; Phases 2 *build*, 3, and live-infra are uncommitted on the branch).

**Gates re-run today, both rings, full:**

| Gate | Result |
|---|---|
| Backend pytest (fast) | ✅ **3234 passed**, 12 skipped, 70 deselected |
| Frontend `tsc --noEmit` | ✅ clean |
| Frontend vitest | ✅ **688 passed / 60 files** |
| Backend wire-drift | ✅ **11/0** |
| Frontend `check:wire-types` | ✅ no drift |
| Mem0 **live** smoke (`scripts/mem0_smoke.py`) | ✅ **7/7** against Mem0 Cloud |

**Per-workstream status:**

| Workstream | Status | Evidence / notes |
|---|---|---|
| **Phase 1 — runtime recall + store** | ✅ **DONE + committed** (`935433e`) | recall in `route_node` (universal seam, memoized, T1/T2/T3), store in `reasoning_recap`, `MEMORY_ENABLED` flag default-OFF, two BlackBox carriers, cross-user-leak guard. |
| **Phase 2 — typed auto-capture (shadow-first)** | ✅ **build DONE, uncommitted**; ⏳ **eval calibration deferred** | `TypedMemory` + `MemoryExtractor` + `prompts/memory_extractor.j2` + `MemoryAutoCaptureService` (shadow: proposes + emits carrier, stores nothing) + `MEMORY_AUTOCAPTURE_ENABLED` default-OFF. Write-back stays gated until the grounded-theory enable-policy clears (Stage 0–7, needs collected shadow traces). |
| **Phase 3 — memory panel + recall indicator** | ✅ **DONE, uncommitted** | `MemoryStore` port + `HttpMemoryStore` (forwards over bearer, F-R9) + BFF `/api/memory` + `MemoryPanel` + `RecallIndicator`. |
| **Phase 3 — chat-history sidebar** | ✅ **DONE, uncommitted** | user-scoped list/rename/archive, deterministic `derive_thread_title`, time-grouping, mounted in `chat-shell.tsx`. |
| **Phase 3 — click-to-resume** | ✅ **DONE, uncommitted** | `fetchThread` + `threadMessagesToTurns` + `resumeThread` (binds `threadIdRef` so the next send continues the resumed checkpoint). |
| **Phase 3 — recall-count domain event** | ✅ **DONE, uncommitted** | `MemoryRecalled(count)` domain event (`extra='forbid'` rejects content) → Custom channel → `recalledCount` on the run view → live indicator (was literal `0`). |
| **Live-infra A — prod CRUD routes** | ✅ **DONE, uncommitted** | `/agent/threads` + `/agent/memory` mounted in `app_prod.py`, shared `_resolve_identity`/`_bearer_identity`, owner-scoped, 503 when memory unwired. |
| **Live-infra B — durable Mem0 backend** | ✅ **DONE + LIVE-VALIDATED, uncommitted** | `Mem0MemoryBackend` (sync Protocol), composition selects on `MEM0_API_KEY`. **The live smoke caught + fixed 3 prod-breaking SDK-2.x bugs** (filters/envelope, `infer=False`, JSON-string metadata) — see the 2026-06-18 block below. |
| **Live-infra C — persistent Neon ThreadStore** | ✅ **code DONE, uncommitted**; ⏳ **not yet live-exercised** | `NeonThreadRepo` + Drizzle schema + hand-authored migration + `selectThreadRepo` wired into `serverPortBag`; F-R2 SDK confinement hardened. Needs `DATABASE_URL` materialized (Terraform-synthesizes it from `NEON_API_KEY` — see the secrets section) + `psql` migration applied. |
| **TDD review of live-infra** | ✅ **PASS** | `docs/reviews/memory_live_infra_tdd_review.md` — TAP-1..4 clear, L1–L4 map correct; LOW #2 (live Mem0 smoke) now RESOLVED. |

> **➕ HERMES / MEMORY-OS ADOPTIONS (A1/A2/A3) IMPLEMENTED 2026-06-19 (uncommitted on this branch).** A separate, design-doc-driven extension landed this session — bounded budget + deterministic consolidation (A1), relevance-floor + dedup recall (A2), and salience-provenance tiers (A3) — all behind default-OFF flags, prod byte-identical until flipped. Full status, **critical review (weaknesses + risks)**, the 5 mid-session decisions taken, and the prioritized remaining-TODO ledger live in [`docs/research/memory/hermes_adoptions_design.md`](../research/memory/hermes_adoptions_design.md) **§10**. Gates this session: backend **3340 passed**, frontend **710 passed**, architecture 99, determinism 10× no-flake, wire-drift regenerated. **Live cloud validation IN PROGRESS** (backend `mem-hermes` rev 00088-gon + `memui` FE rev 00057-qah, both `--no-traffic`; `MEM_SMOKE` authed run mid-flight). **P0 follow-ups (must finish):** complete the smoke → `analyze_memory_traces.py --gate` (investigate a `dom_recall=1` signal on the leak/abstention CONTROL cases against the trace carrier) → governance-audit → **RESTORE `memui`** (cmd in `cache/memory_multisession/MEMUI_RESTORE.txt`) + tear down `mem-hermes`. NOTE: A1 consolidation only "bites" in prod once autocapture write-back flips (still behind the Track-2 enable-policy below).

**Parallel tracks in flight (2026-06-18):**
...
> placeholder, **no traffic**, nothing routes to it) and wrongly reported "BLOCKED —
> placeholder." That service is irrelevant to the memory deploy; the prod backend is
> `agent-backend-combined` and the prod BFF is `agent-frontend` (Cloud Run, **not**
> Cloudflare Pages — Cloudflare dev-tier IaC removed 2026-06-18). `deploy_piece_c.sh`
> + `DEPLOY_PIECE_C.md` corrected to the right service/secret names; the
> `--image`-required guard is kept (defends fresh services + the orphan).
>
> *Second correction (2026-06-18):* the BFF `DATABASE_URL` binding is **not a simple
> secret task** — `database-url` is a **Cloud SQL** `/cloudsql/` socket DSN, and the
> only prod `ThreadRepo` (`NeonThreadRepo`) is hard-wired to the **Neon HTTP driver**
> (`@neondatabase/serverless`), which **cannot dial a Cloud SQL socket**. Binding it on
> `agent-frontend` today would **crash the thread queries**. The sidebar stays on the
> in-memory repo until the option-B `pg` adapter lands — see Remaining #3 +
> [`bff_cloudsql_thread_repo.plan.md`](bff_cloudsql_thread_repo.plan.md). (The earlier
...
> **TDD REVIEW PASSED 2026-06-18 — [docs/reviews/memory_live_infra_tdd_review.md](../reviews/memory_live_infra_tdd_review.md).** Reviewed the three live-infra pieces against [docs/reviews/TDD_AGENTS_MD_REVIEW.md](../reviews/TDD_AGENTS_MD_REVIEW.md)'s shipped guidance (anti-patterns TAP-1…TAP-4, the L1–L4 test-category map, test-dependency rule). Verdict ✅ PASS, no blockers: **TAP-1** (tautological) clear — tests assert behavior, never re-implement the `(user_id,key)`↔Mem0 mapping or keyset math (`grep __ltm_` in mem0 test = 0); **TAP-2** (mock addiction) clear — B + C use ZERO mocking-library calls (behavioral fakes); A's 54 MagicMocks are all at the composition seam (GCS/JWT/adapters/Postgres = truly-external infra TAP-2 exempts) while the units-under-test (`_ThreadStore`, real `LongTermMemoryService(InMemoryMemoryBackend())`) stay real, incl. a shared-backend owner-isolation test; **TAP-3** (determinism theater) N/A — no LLM in the live-infra layer; **TAP-4** (gap blindness) **exemplary** — failure paths written first in all three (401/503/404/cross-user-leak rejection before any happy-path roundtrip). L1–L4 map correct (mem0 = L2 contract, app_prod = adapter/authz-matrix, neon = adapter); test-dependency rule holds. **Two LOW follow-ups** (not blockers): (1) add a test that a typeless stored memory still lists cleanly (only reachable via the durable backend with externally-created rows); (2) treat the live Mem0 smoke test as REQUIRED before prod promotion since CI never exercises `_client()`'s real `from mem0 import` branch (correct CI posture, but the live run is the only validation the real SDK shape matches the fake). Fixed one INFO item inline (stale `lib/db/schema.ts` docstring ref in `neon_thread_repo.ts` → `lib/adapters/thread_store/db/schema.ts`).
>
> **MEM0 LIVE SMOKE TEST RUN 2026-06-18 — caught + fixed 3 prod-breaking adapter bugs; Piece B now live-validated.** Ran the live exercise (`scripts/mem0_smoke.py`, put→get→search→delete under a throwaway user_id) against Mem0 Cloud with the real `.env` key. Installed SDK is **mem0ai 2.0.4**; the adapter + its `_FakeMem0Sdk` were written to the 1.x surface, so 13/13 unit tests passed while the adapter was fully broken live — exactly the TDD-review LOW #2 prediction. Three defects fixed TDD-style (corrected the fake to the real 2.x contract first → red → fixed adapter → green → re-ran live to 7/7): **(1)** `get_all`/`search` reject top-level `user_id` (use `filters={"user_id":...}`) + return `{"results":[...]}` envelope (not a bare list) + `search` uses `top_k`; **(2)** `add` defaults to `infer=True` = async LLM extraction that reworks/drops the text — a keyed store needs `infer=False` (verbatim, synchronous); **(3)** Mem0 Cloud **flattens nested metadata** (`{"text":"hi"}`→`"text.hi"`), so structured fields are now stored as **JSON strings** and decoded on read. Live smoke 7/7, unit 14/14 (+infer-false test), composition+app_prod 63/0. LOW #2 in the TDD review is now RESOLVED.
>
> **SECRETS PROVISIONING (verified against live `.env` 2026-06-18) — see the "Live exercise: provisioning the two secrets" section at the end of this plan.** **Piece B (Mem0) is ready now** — `MEM0_API_KEY` + `MEM0_BASE_URL` are both present in `.env` (the sync backend + async BFF client share the one key). **Piece C needs `DATABASE_URL`, which is absent — but `NEON_API_KEY`/`TF_VAR_neon_api_key`/`NEON_DATABASE_NAME` ARE present.** Key distinction: `NEON_API_KEY` is the **control-plane** Terraform token (read only by OpenTofu, `infra/dev-tier/`), NOT a connection string; `DATABASE_URL` is the **data-plane** string the runtime opens, and Terraform *synthesizes* it from the API key + provider password and lands it in Secret Manager (`neon-database-url`). So Piece C's live exercise = `tofu apply` under `infra/dev-tier/` (or a manual console pull) to materialize `DATABASE_URL`, then apply the thread-table migration. The agent ring runs in-memory without these (prod parity holds).
>
> **⚠ SUPERSEDED 2026-06-18 (this block describes the dev-tier Neon path, NOT what prod actually runs).** The live `agent-backend-combined` reads `DATABASE_URL` from the **`database-url`** secret, which is a **Cloud SQL** connection provisioned by **`infra/gcp/`** (`google_sql_database_instance.main`) — *not* the dev-tier Neon `neon-database-url` synthesized above. `database-url` already exists; there is **nothing to `tofu apply`** for it, and the two secrets must **not** be synced (Neon→`database-url` would repoint the live backend onto an empty DB). The thread migration must land in **Cloud SQL** via the Cloud SQL Auth Proxy (`deploy_piece_c.sh` handles this). See the corrected [`DEPLOY_PIECE_C.md`](../deploy/DEPLOY_PIECE_C.md) §DB and the memory note `memory-deploy-placeholder-image-defect`.
>
> **LIVE-INFRA WIRING IMPLEMENTED 2026-06-17 (all 3 pieces, tests-first, uncommitted, on branch).** Lifted the last deferred non-eval item — the memory + thread stores now reach a real prod surface and can persist. Decomposed into three pieces of different risk (user approved "everything"):
> - **Piece A — prod CRUD routes.** `middleware/app_prod.py` previously built its own minimal FastAPI (just `/run/stream`, `/run/cancel`, understanding-edit) and never served the `/agent/threads` + `/agent/memory` surface that `agent_ui_adapter/server.py:build_app` had — so the BFF's `HttpMemoryStore` POST to `${MIDDLEWARE_URL}/agent/memory` 404'd in prod. Added the full thread CRUD (list/create/get/rename/archive) + memory CRUD (list/create/delete) to the combined app, reusing `_ThreadStore` + `derive_thread_title` from the adapter server and `components.memory_service` (reached via the full `_build_agent_components()` bag — the 5-tuple `_build_components()` shim drops it). Extracted a shared `_resolve_identity` (auto-provision) + `_bearer_identity` dependency so the CRUD routes use ONE auth+identity contract with `/run/stream`; every route scopes to `identity.owner` (cross-user-leak guard, never a client-supplied user_id); 503 (not 500) when memory unwired. Tests: `tests/middleware/test_app_prod.py` **+7** (`TestAppProdThreadCrud` 3 + `TestAppProdMemoryCrud` 4, failure-paths-first: missing-bearer 401, memory-unavailable 503, owner-isolation), full file 34/0.
> - **Piece B — durable Mem0 backend.** New `services/memory_backends/mem0.py::Mem0MemoryBackend` implements the SYNC `MemoryBackend` Protocol (`put`/`get`/`search`/`delete`) by talking to the Mem0 SDK directly (the loop's recall/store nodes are sync; the BFF keeps its parallel ASYNC `Mem0CloudClient` — no event-loop bridge hazard). `(user_id, key)` ↔ Mem0 mapping: Mem0 auto-ids + free-text, so our `key`/`payload`/`metadata` ride Mem0's per-memory metadata blob; `get`/`delete`-by-key resolve via a metadata scan; `put` is a delete-then-add upsert. Composition selects it when `MEM0_API_KEY` is present (new `mem0_api_key`/`mem0_base_url` `AgentRuntimeSettings` fields), `InMemoryMemoryBackend` otherwise (dev/tests/CI) — the "swap backend = one composition line" promise (§10), nothing under `orchestration/` or `agent_ui_adapter/` changes. Privacy invariant: content never logged (caplog-asserted). Tests: new `tests/services/memory_backends/test_mem0_backend.py` **13/0** (Protocol conformance + failure-first + privacy) + composition selection **+2**.
> - **Piece C — persistent Neon ThreadStore.** Frontend threads are the BFF's concern (F-R9): `serverPortBag()` previously always fell back to `InMemoryThreadRepo` (threads vanished on restart) because no real repo was ever passed. Built `lib/adapters/thread_store/neon_thread_repo.ts::NeonThreadRepo` (implements the narrow `ThreadRepo`, keyset pagination, archive filter, error-translation to `ThreadStoreError` — NO vendor type escapes, A4/F-R8) over a thin `neonDrizzleDb()` SDK seam (drizzle-orm/neon-http + @neondatabase/serverless); the Drizzle schema (`threads` + reserved `thread_messages`, IR-NEON-5 checkpoint tables ABSENT) + a hand-authored `0000_init_threads.sql` migration (drizzle-kit still not a dep). **Co-located all drizzle/neon code under `lib/adapters/thread_store/db/`** (was `lib/db/`) so the SDK imports stay inside `lib/adapters/**` (F-R2); updated `drizzle.config.ts` + its test accordingly, and **hardened the layering test's SDK-confinement to catch subpath imports** (`drizzle-orm/pg-core` was dodging the exact-string match). `selectThreadRepo(env)` → `NeonThreadRepo` when `DATABASE_URL` set, else in-memory; wired into `serverPortBag`. Tests: new `neon_thread_repo.test.ts` **10/0** (failure-first + mapping + pagination + selector + end-to-end through NeonFreeThreadStore) + adapter-conformance PAIRS row (proves it implements ThreadRepo AND confines the SDK). **Gates (both rings, full):** backend **3233/0** (12 skip), frontend vitest **688/60**, tsc clean, architecture **86/0** (4 files), wire-types drift clean. **STILL DEFERRED:** Phase-2 eval calibration (Stage 0–7, needs shadow traces); commit (user handles); the live *exercise* of Mem0/Neon needs `MEM0_API_KEY` + `DATABASE_URL` + a `psql`-applied migration on a `--tag` no-traffic Cloud Run revision (prod untouched).
>
> **PHASE 3 (recall-count DOMAIN EVENT) IMPLEMENTED 2026-06-17 (cross-ring, tests-first, uncommitted, on branch).** Closed the last UI-behavior gap: the transparent-recall indicator now shows the REAL per-turn recall count (was a literal `0`). Full chain wired across BOTH rings, riding a Custom channel exactly like the `reasoning_summary`/`task_understanding` precedent (zero AG-UI wire-schema change): **Backend** — `route_node` now persists `recalled_memories_count` (metadata only, NEVER content) into `AgentState` (new field) + its return, memoized alongside `recalled_memories` on `recalled_memories_task_id` (a reflexion lap reuses it); new `MemoryRecalled(count)` domain event (`extra='forbid'` → a `content` field is structurally REJECTED, privacy invariant enforced not just by convention) added to the `DomainEvent` union (12→13 members); `domain_to_ag_ui` maps it to `Custom(name="memory_recalled", value={count})`; `langgraph_runtime` emits it from the route node's output, deduped on the count via `_last_recall_count` (reset per run) so it fires once per run not once per lap; telemetry bridge `_SKIPPED_TYPES` (it's a pure UI affordance — the Recording pillar already has the `MEMORY_RECALLED` BlackBox carrier + `memory_recall` eval_capture, re-exporting to Langfuse would only add noise). **Frontend** — new `MemoryRecalledEvent` on the `UIRuntimeEvent` discriminated union; `ag_ui_to_ui_runtime` special-cases `memory_recalled` (count only; negative/non-integer/malformed → emits nothing); `run_view_reducer` folds it into a new `recalledCount` field on `AssistantRunView` (last-write-wins, survives terminal freeze); ChatShell swaps `count={0}` → `count={turn.assistant.recalledCount}`. **THREE backend count-guards updated** (12→13): two in `test_domain_events.py` + the A3 Event-Completeness guard in `test_telemetry_correlation.py` (added `MemoryRecalled` to `SKIPPED_TYPES` + `_make_event`). **Wire artifacts regenerated** (`scripts/regenerate_wire_artifacts.sh`): the regen also swept in PRE-EXISTING drift from earlier Phase-3 work (the memory CRUD + thread list/rename models + PATCH/DELETE paths had been added but `openapi.yaml`/`wire-types.ts` never regenerated) — all legitimate, drift tests now green. Tests (failure-paths-first): backend domain-events **+4** / translator **+2** / runtime-adapter **+3** (`TestMemoryRecalledChainEnd`: once-per-run dedupe, missing-count-noop, other-node-ignored) / memory-wiring **+3** (count surfaced on state, 0 when flag off, 0 on degraded backend); new interactive jsdom `recall_indicator_flow.test.tsx` **3/0** (no-event→no-indicator, count-0→quiet, positive→"Recalled N memories" + asserts no content leaks) + reducer **+4** + ag_ui translator **+4**. Gates green: backend affected dirs **702/0**, orchestration+architecture **272/0**, wire-drift **11/0**; frontend vitest **675/0** (59 files), tsc clean, arch **83/0**, wire-types drift clean. **STILL DEFERRED:** live infra (prod `app_prod.py` routes + persistent Postgres ThreadStore + Mem0 backend) + Phase-2 eval calibration + commit. **All Phase-3 UI behavior is now complete** (panel + sidebar + resume + recall-count); only live-infra + eval + commit remain.
>
> **PHASE 3 (click-to-RESUME) IMPLEMENTED 2026-06-17 (seam + tests, uncommitted, on branch).** Lifted the previously-deferred resume item: selecting a past thread in the sidebar now **replays its persisted history into the chat view and continues that checkpoint on the next send** (was: highlight-only). Three new pieces, no new ports: (1) `use_chat_sidebars.ts` gained a pure `fetchThread(fetchImpl, id)` (GET `/api/threads/:id`, Zod-validate `ThreadStateSchema`, BFF's missing/not-owned→404 surfaces as `ChatSidebarsError`) + a pure `threadMessagesToTurns(messages)` translator that pairs the backend's persisted `{role,content}` LangGraph message dicts (`langgraph_runtime.get_state`) into `ChatTurn[]` with **frozen `complete` assistant views** (history must never animate a phantom run; non-string content coerced to ""; system/tool roles dropped; trailing lone user msg tolerated) + a `loadThreadTurns(id)` hook method (fetch+translate, returns `[]` and sets `error` on failure — a broken resume degrades to "nothing happened", never a throw). (2) `useAgentRun` gained `resumeThread(threadId, replayTurns)`: aborts any in-flight run, clears pause/edit/busy, **binds `threadIdRef.current = threadId`** (so the next `send`'s `??=` keeps the resumed id, not a fresh uuid — the load-bearing continuation guarantee), and replaces displayed turns. (3) ChatShell's `onSelect` → `setActiveThreadId` + `sidebars.loadThreadTurns(id).then(resumeThread)`; shell stays pure wiring (F-R1 — the BFF fetch lives in the sidebars hook, not the component). Tests: `use_chat_sidebars` **23/0** (+9: fetchThread failure-first + 6 translator cases), new interactive jsdom `resume_thread_flow.test.tsx` **3/0** (failure-fetch-still-binds-id, full replay+continuation-reuses-thread-id, active-row aria-current — full ChatShell flow via injected sidebars+runtime, mirrors `understanding_edit_flow.test.tsx`). Full gates green: vitest **664/0** (58 files), tsc clean, architecture **83/0**, wire-types drift clean. **STILL DEFERRED:** recall-count domain event (literal 0); live infra (prod `app_prod.py` routes + persistent Postgres ThreadStore + Mem0 backend); commit.
>
> **PHASE 3 (chat-shell MOUNT of both UIs) IMPLEMENTED 2026-06-17 (seam + tests, uncommitted, on branch).** Mounted both Phase-3 surfaces into the live chat shell (`frontend/app/chat-shell.tsx`). Layout went from a single rows-grid (header/messages/composer) to header + a `lg:grid-cols-[auto_1fr_auto]` body: **ThreadSidebar** (left, hidden below `lg`) · chat column (center, `grid-rows-[1fr_auto]` = scroll + pinned composer, with **RecallIndicator** above each assistant turn) · **MemoryPanel** (right, hidden below `lg`). Data source = NEW `components/chat/use_chat_sidebars.ts` hook (the browser analog of `useAgentRun`): owns thread+memory list state, fetches the **same-origin BFF routes** `/api/threads` + `/api/memory` over **cookie auth (NO bearer token — the bearer hop is BFF→middleware, F-R9)**, Zod-validates every response → typed `ChatSidebarsError`, exposes `renameThread/deleteThread/addMemory/deleteMemory/setMemoryEnabled`; load/mutate logic is six exported pure `fetch`-injected functions (node-testable without a DOM, mirrors `consumeRunStream`). **BFF gap filled** (was missing — the sidebar half built the ThreadStore `rename`/`archive` but no route reached them): new `makeThreadRenameHandler` (PATCH, `ThreadRenameRequest` validation, catches the ThreadStore throw → 404 no-existence-oracle) + `makeThreadArchiveHandler` (DELETE, idempotent 204) in `lib/bff/handlers.ts`; wired `PATCH`/`DELETE` into `app/api/threads/[id]/route.ts`. **Latent regression fixed:** the sidebar-half wire change (ThreadState gained required-with-default `title`/`archived_at`) left `copilotkit_ui_runtime.test.ts`'s THREAD literal type-incomplete — tsc now flagged it; added the two fields. ChatShell takes an injectable `sidebars?` test seam (mirrors `runtime?`) so SSR/unit tests render without a network. Tests green: BFF handlers **26/0** (+7 rename/archive), `use_chat_sidebars` **14/0** (new, failure-paths-first), chat-shell **15/0** (+5 mount tests: sidebar+panel render with data, title-not-id, toggle state, empty state, composer survives), full frontend lib+components+app **652/0**, architecture **83/0**, tsc clean, wire-types drift clean. **DEFERRED (next, needs new machinery):** click-to-**resume** a past thread — selecting a row only highlights `activeThreadId` today; replaying a stored thread's turns into the chat view needs a `/threads/[id]` fetch + reducer-replay + a thread-id setter on `useAgentRun` (none exist yet). **Recall-count** still 0 (literal) — the `MEMORY_RECALLED` carrier isn't surfaced on the run-view stream yet (the indicator is mounted but inert; swap the literal when the domain-event path lands). Live infra (prod `app_prod.py` routes + persistent Postgres ThreadStore + Mem0 backend) and commit also still deferred.
>
> **PHASE 3 (chat-history sidebar half) IMPLEMENTED 2026-06-17 (seam + tests, no new DB infra, uncommitted, on branch).** Completed the inert ThreadSidebar scaffold + thread list/title/rename/archive end-to-end. Key scaffold finding: `rowToState` in `neon_free_thread_store.ts` was DROPPING the already-existing `title`/`archived_at` row fields because the wire `ThreadState` had no such fields — so the title existed in the repo but never reached the UI (why the sidebar rendered raw `thread_id`). Fix: added `title` (default "New chat") + `archived_at` to `ThreadState` (both rings) + `ThreadListResponse`/`ThreadRenameRequest` wire models; `rowToState` now passes them through. Backend: upgraded `agent_ui_adapter/server.py` `_ThreadStore` to user-scoped `list` (newest-first, cursor), `rename`, `archive` (soft-delete via `archived_at`, hidden from list/get), ownership scoping on get (no existence oracle); new `GET /agent/threads` (list, scoped to identity.owner, B6) + `PATCH /agent/threads/{id}` (rename) + `DELETE /agent/threads/{id}` (archive) endpoints. Deterministic title: NEW pure `derive_thread_title()` helper (truncate ~60 chars at word boundary + ellipsis, collapse whitespace, fallback "New chat"; NO LLM — an LLM auto-title is a later upgrade behind the same field) set on create from `metadata["first_message"]`. Frontend: NEW pure `lib/thread_grouping.ts` `groupThreadsByTime` (Today/Yesterday/Previous 7 days/Older, clock injected, newest-first, framework-clean); rewrote `ThreadSidebar.tsx` to render TITLE (not thread_id), grouped, with active state + onSelect-resume + per-row rename(window.prompt)/delete callbacks (typed props, no domain logic, F-R1). **THREE drift/test guards forced updates** (resolved): `agent_protocol.test.ts` ThreadState round-trip (now includes title/archived_at + a defaults test), the TS↔Python `baseline_drift.test.ts` → regenerated `__python_schema_baseline__.json` via `npm run wire-schema-snapshot` (+ openapi.yaml/wire-types.ts via regenerate script; new thread paths hand-added to export_openapi). Tests green: backend full fast suite **3200 / 0** (memory-half baseline 3183, +17 = title helper 8 + sidebar endpoints 9); frontend lib+components **434 / 0** (+14 = grouping 5 + sidebar 8 + defaults 1), frontend architecture **83 / 0**, wire drift green, tsc clean. **Deferred (per user "build + test, defer mount"):** mounting `ThreadSidebar` in `chat-shell.tsx` + the RSC/page thread-list fetch + click-to-resume wiring to the runtime (touches the live chat shell); wiring `/agent/threads` list/rename/archive into the prod `app_prod.py` surface + the persistent Postgres/Drizzle ThreadStore swap (live infra); commit.
>
> **PHASE 3 (memory half) IMPLEMENTED 2026-06-17 (seam + tests, no new DB infra, uncommitted, on branch).** Built the editable **memory panel + transparent-recall indicator** half of Phase 3 (chat-history sidebar half deferred per user). Backend: `agent_ui_adapter/server.py` gained `GET/POST/DELETE /agent/memory` over the injected `LongTermMemoryService`, scoped to `identity.owner` (cross-user-leak guard — never a client-supplied user_id; 503 when memory not wired, not 500); wire models `MemoryItem`/`MemoryListResponse`/`MemoryCreateRequest` (`extra=forbid`, no user_id on the create body). Frontend (ports-and-adapters): new `MemoryStore` port (list/add/remove, distinct from the read-only `MemoryClient` recall port — P1 one-interface-per-module) + `HttpMemoryStore` adapter that FORWARDS to middleware over the caller's bearer token (F-R9, no Mem0 creds in BFF, Zod-validates every response); registered in `composition.ts` PortBag; Zod wire schemas mirroring the Python models (W6); BFF `makeMemoryList/Create/DeleteHandler` (auth-gated, B6) + `/api/memory` + `/api/memory/[key]` routes; `MemoryPanel.tsx` (list-by-type semantic/episodic/procedural, add/delete, memory-on/off toggle — typed props + callbacks, no domain logic, F-R1) + `RecallIndicator.tsx` (pure, count-prop, "recalled N memories", count-only never content, renders nothing at 0). **Four registry/drift guards forced updates** (all resolved): port-conformance `REQUIRED_PORTS` + adapter-conformance `PAIRS` (registry completeness, like the backend enum consumers), `composition.test.ts` port-bag key assertion, and the **OpenAPI drift** test → regenerated `openapi.yaml` + `frontend/lib/wire-types.ts` via `scripts/regenerate_wire_artifacts.sh` (memory paths hand-added to `export_openapi._build_paths`; the models auto-discover). Tests green: backend adapter+arch 462, full backend fast suite **3183 / 0 fail** (P2 baseline 3174, +9); frontend lib+components **420 / 0**, frontend architecture **69 / 0**, tsc clean. **Pre-existing unrelated failure noted:** `frontend-explainability/tests/architecture/test_port_conformance.test.ts` fails on a broken `http_explainability_client` import (committed `6ae77cd`, a SIBLING project, NOT mine). **Deferred (per user "seam + tests"):** wiring `/agent/memory` into the prod `app_prod.py` middleware surface + the persistent ThreadStore + the Mem0 backend = the live-infra step; the **chat-history sidebar** half of Phase 3; mounting `MemoryPanel`/`RecallIndicator` in the chat shell + the recall-count domain-event path; commit.
>
> **PHASE 2 IMPLEMENTED 2026-06-17 (build + eval scaffolding, committed/uncommitted per your commit).** Shadow-first typed background auto-capture. Build half: `components/schemas.py::TypedMemory` (Pydantic, `extra="forbid"` = schema-as-classifier guard, `type` Literal semantic/episodic/procedural, salience 0–1); `components/memory_extractor.py` (`MemoryExtractor.extract`, framework-clean, mirrors `TaskUnderstandingGenerator`, ADD-only, raises on whole-response failure but DROPS a malformed item so one junk item can't lose the batch, returns `ExtractionResult` with real token/cost for `eval_capture`); `prompts/memory_extractor.j2` (three typed schemas, conservative "most turns aren't worth remembering"); `LongTermMemoryService.search` optional `mem_type` filter (additive, over-fetch-then-trim so off-type matches don't starve the limit); `services/memory_autocapture.py` (`MemoryAutoCaptureService` — post-run, per-thread debounce, **shadow** by default = emits one `MEMORY_STORED` carrier per PROPOSED item with `proposed_only: true` + `{user_id,key,type,salience}` NEVER content, stores NOTHING; write-back only when `MEMORY_AUTOCAPTURE_ENABLED` flips after the eval enable-policy clears; depends on a local `_ExtractorPort` Protocol — NOT a `components` import, I-5); `memory_autocapture_enabled` flag in base_config + composition; extractor+service constructed at the composition root (extractor via `importlib` string-ref so middleware keeps no static `components` import, F4); optional `autocapture` param on `LangGraphRuntime` wired post-run (off the hot path, skips on error/empty window, default None = byte-identical). Eval scaffolding (no live data): `docs/recipes/memory_extractor/` failure-taxonomy skeleton + gold-set spec (`memory-extract-gold-v1`) + label-sheet CSV + enable-policy doc. **Three architecture guards forced real fixes** (all resolved): services-no-components (→ `_ExtractorPort` Protocol), middleware-no-components (→ importlib string-ref), and the M-Phase2 swap-radius test (service+adapter co-change — legitimate cross-cutting feature, NOT a backend swap; passes on a clean committed diff, **will fire when P2 is committed** since service+adapter change in one range — commit note: skip `-k 'not swap_radius'` or split commits). Tests green: TypedMemory 11, extractor 12, autocapture 10, type-filter 4, runtime seam 4; full fast suite **3174 passed / 0 failed** (P1 baseline 3133); architecture 99 passed; determinism 55×10 no-flake. **Still deferred:** `Mem0MemoryBackend` (ask-first); the eval *calibration* run (needs collected shadow traces — Stage 0–7); Phase 3 UI; commit.
>
> **PHASE 1 IMPLEMENTED 2026-06-17 (committed `935433e`).** Test-first build complete and green. Spike resolved → sync `LongTermMemoryService` route (see Wiring note). Built: `components/memory_context.py` (OBP-1 `render_recall_block`/`build_store_payload` + OBP-2 `should_recall` + `memory_subject` cross-user-leak guard); recall in `route_node` (universal seam, memoized, `MEMORY_RECALLED` carrier + `eval_capture` + graceful-degrade via `await asyncio.to_thread`); `call_llm`/`supervisor` read `recalled_memories`; `_route_fanout` Send payload (OBP-M1); store in `reasoning_recap_node` (`_maybe_store_memory`, reads `last_final_answer` = tier-agnostic, `MEMORY_STORED` carrier); two non-required `EventType` members; `AgentState` fields; `AgentConfig.memory_enabled=False`; composition `MEMORY_ENABLED` flag + constructs `LongTermMemoryService(InMemoryMemoryBackend())`; `langgraph_runtime` threads `user_id=identity.owner`. **Two enum-completeness consumers updated** (required, surfaced by the full suite): `black_box_publisher` mapping (`memory.recalled`/`memory.stored` → `span`) + `dev_seed` corpus. **Real bug a failure-path test caught:** `"anonymous"` must not be a memory subject (cross-user pooling) → `memory_subject()` guard. Tests green: L1 20/20, L4 wiring 7/7, T3 sim `test_fanout_is_not_memory_blind`, all enum-consumer + drift-guard 170/170, full fast suite no-regression. **`Mem0MemoryBackend` deferred** (ask-first new horizontal service; default-OFF flag keeps prod parity meanwhile). **Still pending:** governance-trace-audit gate (Verification 4), local e2e (Verification 5), commit.
>
...
>
> **Hermes / memory-os extension**: [docs/research/memory/hermes_adoptions_design.md](../research/memory/hermes_adoptions_design.md) (trade-off + design for A1 bounded-budget/consolidation, A2 relevance-floor recall, A3 salience tiers) and its implementation plan. Extends this pipeline; does not replace it.
>
> **Compliance backing**: [docs/Architectures/](../Architectures/). Each phase below carries the paste-into-PR checklist from [BACKEND_PR_CHECKLISTS.md](../Architectures/BACKEND_PR_CHECKLISTS.md) (Checklists 2/4/7) and the frontend invariants from [FRONTEND_ARCHITECTURE.md](../Architectures/FRONTEND_ARCHITECTURE.md) (F-R1..F-R9) + the rule families in [STYLE_GUIDE_FRONTEND.md](../style-guides/STYLE_GUIDE_FRONTEND.md). Invariant IDs (I-1..I-14) are defined in [BACKEND_SOLUTION_ARCHITECTURE.md](../Architectures/BACKEND_SOLUTION_ARCHITECTURE.md).
>
> **Phases:**
...
4. **Governance trace check:** run a local task with memory ON, then use the `governance-trace-audit` skill on the resulting trace to confirm `MEMORY_RECALLED`/`MEMORY_STORED` carriers appear and the four-pillar verdict stays healthy. For deployed runs, fetch the trace with [`scripts/fetch_memory_trace.py`](../../scripts/fetch_memory_trace.py) (see [`fetch_memory_trace.plan.md`](fetch_memory_trace.plan.md)).
...
Phase 3 spans two rings. **Backend half** (thread-list endpoint, persistent `ThreadStore`, memory CRUD over `LongTermMemoryService`) → **[Checklist 7](../Architectures/BACKEND_PR_CHECKLISTS.md#7-adding-a-new-adapter-family) / 4**. **Frontend half** (`ThreadSidebar`, `MemoryPanel`, BFF routes) → **[FRONTEND_ARCHITECTURE.md](../Architectures/FRONTEND_ARCHITECTURE.md) F-R1..F-R9** + the rule families in **[STYLE_GUIDE_FRONTEND.md](../style-guides/STYLE_GUIDE_FRONTEND.md)**.
...
## Live exercise: provisioning the two secrets (`MEM0_API_KEY` + `DATABASE_URL`)

The code is in place and **default-safe**: with neither secret set, the agent ring uses `InMemoryMemoryBackend` (Piece B) and the BFF uses `InMemoryThreadRepo` (Piece C) — prod parity holds, nothing 500s. To actually *exercise* durable memory + persistent threads you need both secrets. **As of 2026-06-18 neither is in `.env`/`.env.local`** (root `.env.local` carries only the four `WORKOS_*` keys; root `.env` has the rest of the agent config). The steps below get them. **Never commit either secret** — they go in `.env.local` / `.env` (git-ignored) for local runs, and in **Secret Manager** for Cloud Run (`app_prod.py`'s header already lists both as Secret-Manager-injected).

> **Inventory verified 2026-06-18 against the live `.env`:** `MEM0_API_KEY` ✅, `MEM0_BASE_URL` ✅, `NEON_API_KEY` ✅, `TF_VAR_neon_api_key` ✅ (same value), `NEON_DATABASE_NAME` ✅ (= `neondb`). `DATABASE_URL` ❌ absent — **by design: it is synthesized by Terraform from `NEON_API_KEY`, not pasted by hand** (see §B). Net: **Piece B (Mem0) is ready now**; Piece C needs one `tofu apply` (or a manual pull) to materialize `DATABASE_URL`.

### Two planes — don't confuse the Neon keys

`NEON_API_KEY` is **not** a database connection string. There are two distinct planes, and each key lives on exactly one:

| | Control plane | Data plane |
|---|---|---|
| **Key** | `NEON_API_KEY` / `TF_VAR_neon_api_key`, `NEON_DATABASE_NAME` | `DATABASE_URL` |
| **What it is** | a console.neon.tech API token + the db name | `postgresql://user:pass@host/db?sslmode=require` |
| **Who reads it** | **OpenTofu/Terraform only** (`infra/dev-tier/versions.tf:65`, `variables.tf:102`, `neon.tf:78`) — authenticates the `kislerdm/neon` provider to adopt/manage the project | the **runtime** — `NeonThreadRepo`/`neonDrizzleDb` (Piece C) open it; the LangGraph checkpointer also uses it |
| **How obtained** | from the Neon console (API Keys); already in `.env` | **derived by Terraform** from the API key + the provider-returned password, then stored in Secret Manager (`neon-database-url`) and injected into the `middleware` Cloud Run service (`neon.tf:123`, `secret-manager.tf:210`, `cloud-run.tf:214`) |

The `TF_VAR_` prefix is Terraform's convention for auto-binding an env var to the `neon_api_key` input variable — that's the *only* reason both the prefixed and bare copies exist.

### A. `MEM0_API_KEY` (Piece B — durable long-term memory) — ✅ already in `.env`

`Mem0MemoryBackend` talks to **Mem0 Cloud** (the same managed service the BFF's async `Mem0CloudClient` already targets). Both `MEM0_API_KEY` and `MEM0_BASE_URL` are present in `.env`, and the sync backend + async client construct the *same* underlying SDK client, so **the one key already serves both rings**. Nothing to provision for Piece B locally — just confirm `MEMORY_ENABLED=true` is set when you want recall/store to actually fire (default OFF for prod parity).

- `MEM0_BASE_URL` is already set; default is `https://api.mem0.ai` (the v2 self-hosted profile uses `https://mem0.internal`).

Cloud Run (no-traffic stress revision, prod untouched):
```bash
# one-time: store the secret
printf 'm0-…' | gcloud secrets create MEM0_API_KEY --data-file=-   # or: versions add
# deploy a tagged, no-traffic revision that maps the secret + flips memory on
gcloud run deploy <service> --tag mem --no-traffic \
  --update-secrets=MEM0_API_KEY=MEM0_API_KEY:latest \
  --update-env-vars=MEMORY_ENABLED=true
```
Verify selection in logs: the composition root logs **`memory backend: mem0 (durable)`** when the key is present, **`memory backend: in-memory (ephemeral)`** when it isn't ([middleware/composition.py:709–719](../../middleware/composition.py)).

> **CI stays keyless on purpose.** The mem0 SDK import path (`_client()` → `from mem0 import MemoryClient`) is never exercised in tests (only the injected-`sdk_client` fake is). So the **first live run with a real key is the only validation that the real SDK surface matches the fake's shape** — treat it as a REQUIRED smoke step before any prod promotion (TDD-review LOW #2).

### B. `DATABASE_URL` (Piece C — persistent threads on Neon Postgres) — ❌ absent, materialize from `NEON_API_KEY`

`selectThreadRepo(env)` returns `NeonThreadRepo(neonDrizzleDb(url))` when `DATABASE_URL` is set (non-blank), else `InMemoryThreadRepo`. The driver is `@neondatabase/serverless` over `drizzle-orm/neon-http`, so the URL must be a **Neon** Postgres connection string. `NEON_API_KEY` cannot be used directly — it's the control-plane token (see the two-planes table above); the connection string is *derived* from it. Two ways, repo-native first:

1. **Repo-native (Terraform-driven, recommended — never see the password):** `NEON_API_KEY`/`TF_VAR_neon_api_key`/`NEON_DATABASE_NAME` are already in `.env`, so a `tofu apply` (or `tofu plan` to preview) under `infra/dev-tier/` adopts the existing `agent-prod-dev` project, derives `local.neon_database_url` from the provider-returned password (`neon.tf:123`), enables `pgvector`, lands it in Secret Manager (`neon-database-url`), and injects it into the `middleware` Cloud Run service as `DATABASE_URL` (`cloud-run.tf:214`). For a *local* value, pull it back after apply:
   ```bash
   gcloud secrets versions access latest --secret=neon-database-url   # → the connection string
   ```
2. **Manual (no Terraform):** in the Neon console (project `agent-prod-dev`), **Connection Details** → copy the **pooled** string (the neon-http driver wants the `…-pooler.…neon.tech` host): `postgresql://neondb_owner:<pass>@ep-…-pooler.<region>.neon.tech/neondb?sslmode=require`.

Then, regardless of which path produced the URL:

3. **Apply the thread-table migration** (drizzle-kit is intentionally not a dependency — apply the hand-authored SQL directly; idempotent `CREATE TABLE IF NOT EXISTS`):
   ```bash
   psql "$DATABASE_URL" -f frontend/lib/adapters/thread_store/db/migrations/0000_init_threads.sql
   ```
   Creates `threads` + the partial index + the reserved `thread_messages` table. **Deliberately omits the 4 LangGraph checkpoint tables (IR-NEON-5)** so it coexists with the checkpointer's tables in the same `neondb`.
4. **Local BFF use** — the Next.js BFF reads it from `frontend/.env.local`:
   ```bash
   echo "DATABASE_URL=$(gcloud secrets versions access latest --secret=neon-database-url)" >> frontend/.env.local  # do NOT commit
   ```
5. **Cloud Run / Vercel BFF:** map `DATABASE_URL` from the `neon-database-url` secret on the BFF service revision (same `--tag … --no-traffic` discipline).

> **One database, two consumers — already compatible.** The Terraform already injects `DATABASE_URL` into `middleware` for the LangGraph **checkpointer**, pointing at this same `neondb`. The BFF ThreadStore shares that database (the migration avoids the checkpoint tables), but the value must also reach the **BFF's** environment (`frontend/.env.local` / the BFF service) — the ThreadStore is the BFF's concern (F-R9), not the agent backend's. (`pgvector` is already enabled on `neondb` by `neon.tf:93`, so the future Mem0→pgvector graduation has its substrate ready.)

### C. Live verification checklist (after both secrets are set on a no-traffic revision)
1. **Backend selection** — logs read `memory backend: mem0 (durable)`.
2. **Memory roundtrip** — `POST /agent/memory` then `GET /agent/memory` (bearer auth) returns the item; restart the revision and confirm it survives (the in-memory backend would lose it).
3. **Thread persistence** — create a thread via the UI, restart the BFF, confirm it's still in the sidebar.
4. **Governance** — run the `governance-trace-audit` skill on a memory-ON trace; confirm `MEMORY_RECALLED`/`MEMORY_STORED` carriers present, **content absent**, four-pillar verdict COMPLIANT (plan Verification step 4 — mandatory for telemetry-touching changes).
5. **Privacy** — grep the revision's logs for any stored memory content; there must be none (only `user_id`/`key`/counts).
