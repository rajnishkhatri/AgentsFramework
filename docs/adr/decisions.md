...
- 2026-09-02 — **Exam UI reuses Test Mode timer siblings.** `ExamRunnerView` imports `CountdownTimer` / `format_clock` from `components/test/` (plan §1). Rejected: a second clock formatter in `components/exam/` (connascence drift). Isolation guard still forbids quiz/scheduler/`skill_state` edges only.
- 2026-09-02 — **D1 closed: ADR-0040 Accepted.** Human flipped [ADR-0040](0040-exam-module-durable-runs-analytics.md) Proposed → Accepted (index + log). D2 cost/cross-team/security routing floors stay **UNSET** — [approval-criteria.md](common/approval-criteria.md). Rejected: inventing D2 cells from FR-32/FR-33 analytics floors.
...
- 2026-07-15 — **Eng Coach GCP Q1: FR-6 is satisfied by the `(coach)/layout` guard, not a coach CTA (spec defect corrected at implement time).** The spec/plan assumed a distinct *coach* sign-in CTA at [page.tsx:64](../../frontend/app/page.tsx) to carry `returnTo=/learn` (plan T3, task B2). The tree has **one** web sign-in CTA — the generic agent "Sign in with WorkOS" button — and FR-7 forbids redirecting *it* to `/learn`. Found that [`(coach)/layout.tsx:31`](../../frontend/app/(coach)/layout.tsx)'s `withAuth({ ensureSignedIn: true })` **already** returns an unauthed `/learn` visitor to `/learn` post-login: AuthKit's `redirectToSignIn` calls `getReturnPathname(url)` and threads the current path as `returnPathname` (`@workos-inc/authkit-nextjs` session.js:324). So: **page.tsx untouched** (FR-7 safe by construction; B2 dropped), **B1 kept** (route.ts threads `getSignInUrl({returnTo})` for any future explicit CTA — FR-6/7 tests green), FR-6 now guarded by a layout tombstone (`layout.test.tsx` "returns an unauthed /learn visitor…" — asserts `ensureSignedIn: true` + no `returnTo/returnPathname/redirectUri` override that would defeat native capture). FR-8 already covered by `desktop_auth_state.test.ts:93`. User-confirmed 2026-07-15 (chose "layout guard satisfies FR-6" over building a new coach CTA / routing to sdd-replan). Rejected: a distinct pre-auth coach surface (invents an unauth `/learn` landing the app doesn't have). Spec: [eng-coach-gcp-deploy.spec.md](../plan/eng-coach-gcp-deploy.spec.md) FR-6..8.

- 2026-07-15 — **Eng Coach GCP D7 seed: pure `planEngineSeed` helper resolves the NODE_ENV test-seam.** `browserEngineAdapters()` computes `isProd` once and delegates the seed-pack choice to a pure `planEngineSeed({isProd,seedMode})` → `"none"|"fresh-pack"|"demo-corpus"` ([frontend/lib/engine_seed_plan.ts](../../frontend/lib/engine_seed_plan.ts)). Pure extraction from an existing branch (no new dep/service/node) → no ADR (ADR-0033 covers the seed-gate *lift*). The plan flagged NODE_ENV inlining as a risk; found it does **not** bind under Vitest (this config has no Vite `define` for NODE_ENV, so `process.env.NODE_ENV` is read at runtime and `vi.stubEnv` flips the branch) — the helper is belt-and-braces for the Next prod bundle, which *does* inline. FR-1..5 tested at BOTH layers (helper L1 table + `browserEngineAdapters` prod-branch via `stubEnv`). Rejected: an injectable `isProd` param on the public builder (widens the API for a test-only need the stub already covers). Spec: [eng-coach-gcp-deploy.spec.md](../plan/eng-coach-gcp-deploy.spec.md) FR-1..5.
...
- 2026-07-13 — **WorkOS auth Stage-5 replan (P1).** (1) FR-6 correlator = `thread_id`/`thread=` (audit is pre-stream; no domain `trace_id` yet) — rejected inventing a trace at the seam. (2) FR-6 audit on **both** accept and reject (`verified=True|False`) — rejected success-only (would hide denies). (3) Q-C2 DoD = structural `/api/auth/*` outside `(coach)`; live Tauri/iOS smoke deferred. (4) `E2E_BYPASS_AUTH` explicit edge case (mirrors `page.tsx`). Spec/plan/tasks: [eng-coach-workos-auth](../plan/eng-coach-workos-auth.spec.md).

- 2026-07-13 — **D3 verify target = coach card agent_id, not learner subject.** At the coach `/run/stream` seam, `registry.verify(SUBJECT_COACH_AGENT_ID)` checks the signed coach card (the integrity-bearing artifact). Rejected: verifying the learner subject — subjects are auto-provisioned and not the signed card under test. Spec: [eng-coach-workos-auth.spec.md](../plan/eng-coach-workos-auth.spec.md).
