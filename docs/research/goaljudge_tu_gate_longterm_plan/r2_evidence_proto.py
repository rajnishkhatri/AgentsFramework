    ...
    for i in sample_idx:
        ...
        for s in range(2):
            resp = await llm.invoke(
                profile, [{"role": "user", "content": PROMPT.format(task=task)}]
            )
            ...
            verdict = (
                "PASS" if (n_null <= 1 and not bad and 2 <= len(conds) <= 7) else "FAIL"
            )
            print(
                f"[{i:02d}] s{s} {verdict}  n={len(conds)} null={n_null} nonverbatim={len(bad)}"
            )
            for c in bad:
                print(
                    f"      BAD-QUOTE: '{str(c.get('evidence'))[:60]}' for: {str(c.get('text'))[:60]}"
                )
