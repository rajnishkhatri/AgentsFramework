...
def path_seg(toks):
    ...
    for t in toks:
        if "/" in t or "." in t:
            out |= {
                s for s in re.split(r"[/.]", t) if len(s) >= 2 and s not in _STOPWORDS
            }
    return out
...
def fails(conds, task, xform):
    ...
    return [i for i, c in enumerate(conds) if tt and not (xform(base_tokens(c)) & tt)]
...
XF = {
    ...
    "PSG punct+stem+pathseg": lambda s: (
        {stem(t) for t in path_seg(strip_punct(s))} | path_seg(strip_punct(s))
    ),
}
...
for i, rec in sorted(data["passing"].items(), key=lambda kv: int(kv[0])):
    for s, conds in enumerate(rec["samples"]):
        ...
        other = (
            validate_conditions(conds, rec["task"], source="user_edited")
            if False
            else []
        )
        if f:
...
for n in (30, 101):
    row = "  ".join(
        f"p={p:.2f}:{p_pass(n, p, 0.95):.2f}" for p in (0.93, 0.95, 0.97, 0.98, 0.99)
    )
