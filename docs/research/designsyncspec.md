...
> **Root font size is 17.5px.** `styles.css` sets `html{font-size:17.5px}`. All `rem` values —
> including Tailwind spacing/width classes (`w-64`, `p-4`, container-query thresholds) — resolve
> against this root. Computed px below use **1rem = 17.5px**. If the production app overrides the
> root, recompute the px column; the `rem` column is canonical.
