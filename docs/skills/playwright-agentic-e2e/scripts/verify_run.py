...
def main() -> int:
    ...
    if args.dedupe and dup_ids:
        ...
        print(
            "  (re-runs present — pass --dedupe so the split reflects distinct cases)"
        )
    ...
    if args.expect_cases is not None:
        ...
        ok &= good
        print(
            f"[{'PASS' if good else 'FAIL'}] expected {args.expect_cases} cases, got {n}"
        )
    ...
    if args.id_namespace:
        ...
        ok &= good
        print(
            f"[{'PASS' if good else 'FAIL'}] trace_id == uuid5({args.id_namespace}, case_id)"
    ...
    if args.log_marker_count is not None:
        ...
        ok &= good
        print(
            f"[{'PASS' if good else 'FAIL'}] backend marker lines {args.log_marker_count} == rows {n}"
            + (
                ""
                if good
                else "  ⚠ backend ran a different count than the harness drove"
            )
        )
    ...
    print(
        "  # Dedupe by a per-run id before counting (retries/replicas duplicate lines)."
    )
    print("=" * 64)
    ...
    print(
        ...
        + (
            f" — but {len(status_only)} run(s) showed a UI render gap"
            if status_only
            else ""
        )
    )
