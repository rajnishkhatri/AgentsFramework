...
- [synthetic-data-pipeline](synthetic-data-pipeline/SKILL.md) — stage-gated synthetic-data pipeline for the coach question banks: demand gate → scoped generation → deterministic cascade → solve-consistency/contamination → ISO 2859-1 acceptance sampling (the only place `reviewed=true` is earned) → emit → monitor; owns the Plane-T DEFER decision.
