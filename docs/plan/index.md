...
- [Wide-layout CoachPanel parity spec](preact-wide-layout-coach-panel.spec.md) — Stage-2 EARS **FR-1…20 ≡ locked AC-1…20** (Direction 2b): `coachMode` 900px rule, **64px** rail (no pin), hint ladder + conversation, `CoachDrawer`+pill, Zone C pinned nudge. **Re-Accepted 2026-07-16** (clarify Q-C1…Q-C5 CLOSED). Stage 4 PASSED — human Accept → `sdd-implement`. Pairs with [plan](preact-wide-layout-coach-panel.plan.md) + [tasks](preact-wide-layout-coach-panel.tasks.md) + [ADR-0036](../adr/0036-wide-layout-coach-panel-parity.md). Product lock: [`locked-spec-artifacts/`](../width-design-ui-session-artifacts/locked-spec-artifacts/).
- [Wide-layout CoachPanel parity plan](preact-wide-layout-coach-panel.plan.md) — Stage-2 architecture: `coachMode`, 64px rail, `use_expandable_list`, HintLadderList, CoachDrawer+pill, Zones A/B/C, L0–L6 sequencing.
- [Wide-layout CoachPanel parity tasks](preact-wide-layout-coach-panel.tasks.md) — Stage-4 **PASSED 2026-07-16**: L0–L6 board grounded; `make check` + scoped vitest green. Human Accept → `sdd-implement`.
- [CoachPanel real-estate + Zone-B scroll spec](preact-coach-panel-real-estate.spec.md) — Delta after W9 iPad screenshots: wider coach column (drop `w-80`), revealed nudges + answers share one Zone B scrollport, Zone C stays pinned. **Draft** — clarify OPEN; Accept before plan/tasks.
...
- [Eng-coach Tauri Mac shell brainstorm](eng-coach-tauri-mac-shell.brainstorm.md) — Stage-1 **OPEN**: "create Tauri for GCP eng coach" — premise audit **REFUTES** greenfield scaffold (Tauri 2 + desktop WorkOS deep-link already at `frontend/src-tauri/`, ADR-0001/P5); re-posed as coach-first vs dual-product specialization + packaging bar, sequenced behind sibling D7 usable-`/learn`. Directions D1–D7; gate Q1–Q4.
