...
| **S-F1** | ADR-0040 Proposed→Accepted (index/log) **[D1 ✅ Accepted 2026-09-02]**; `decisions.md` ✓; ADR-0041 ✓. D2 path: [approval-criteria.md](../adr/common/approval-criteria.md) (values UNSET) | — | OKF lint 0 failures; D2 remains human |
| **S-F2** | **R5 infra** — Cloud Run `min-instances ≥ 1` (exam route) + Cloud SQL pool cap **[✅ applied 2026-09-03 — `tofu apply` set frontend `min_instance_count 0→1`; rev `agent-frontend-00080-82q`]** | — | infra applied (Terraform/deploy) |
| **S-F3** | pre-traffic `migrate_engine.mjs` runs `0005`; second-device read-back smoke **[⚠️ 2026-09-03 — migration applied pre-traffic (`{"ok":true}`) + health smoke green (FE 200, `/healthz` ok, `/learn/exam`→307 auth). BUT authenticated live walkthrough FAILED: `/api/engine/db/upsertExamRunItems` → HTTP 500 (`EngineRepoError`, insert on `exam_run_item`) while `insertExamRun`=204 / `beginExamSection`=200 / reads=200. Answers never persist → buffer "not saved" → finalize correctly blocked → no review screen. Root cause = prod `exam_run_item` shape ≠ `0005` (pre-existing table skipped by `CREATE TABLE IF NOT EXISTS`), confirmed locally: code passes on fresh `0005`; a simulated column-drift reproduces the exact `Failed query`; `0006_exam_run_item_reconcile.sql` (DROP+recreate, data-safe — 0 rows ever persisted) fixes it. **FIX APPLIED to prod 2026-09-03** via frontend phase (`applied:["0006_exam_run_item_reconcile.sql"]`, `{"ok":true}`). Post-fix prod reads on `exam_run_item` return 200 + review renders (both fail on the drifted table), so the reconcile took. Clean authenticated WRITE E2E still PENDING: the earlier failed smokes left the E2E user's English run expired/in-review (non-restartable, FR-2), so a fresh runner needs a test-data reset (DB delete = classifier-blocked). Follow-up: exam E2E is not idempotent vs the durable backend.]** | S-F2 | prod smoke (plan §3) |
