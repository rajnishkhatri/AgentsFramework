...
| 3.6 | **Stage 4 — rubric revision** (PROVISIONAL → REVISED) | ✅ DONE (2026-07-04, commits `9362097`+`5a85bf0`) | agent | 3.4 | Both `.j2` REVISED + ADR-0017. Replan finding (ADR-0017 F5–F9): the real blocker was a **data-plane gap** (judge fed bare `question_id`, never the item) + 3 mislabeled positives — fixed via `enrich_coach_judge_cases.py`/recorder + fixture corrections (FR-13/FR-14). Honest 4-run Opus baseline: **0/5 → B1 4/4 + G3 4/4 caught, A3 2/4, A1/A2 attributed, TNR=1.000**. Strict 5/5 recall DEFERRED to the ≥20-trace out-of-sample round. Telemetry-only (flags OFF). |
| 3.7 | **Stage 5 — `coach_goldset_v1` assembly** | ✅ DONE (provisional, 2026-07-04) | agent | 3.6 | Built `CoachGoldsetItem` + `assemble_coach_goldset.py` (mirror GoalJudge; Invariant #7 — `LeakChannel` mirrored locally, no components import; α REUSES `iaa.krippendorff_alpha_nominal` NaN→None). **Provisional artifact** `tests/fixtures/coach_goldset/coach_goldset_v1.json`: 21 scorable rows (22 cases − I1), leak_share 0.24, `provisional=true`, α=null, `rubric_version=coach_rubric_v1_revised`, `taxonomy_version=coach_axial_v1`. Fail-closed (refuses cert). **REMAINING before 3.8:** the human double-label pass over the 292-turn corpus → 200–300 rows + 60/40 frozen test split + **α ≥ 0.80 on `answer_leakage`** clears `provisional`. `make check` green; 25 L1 tests. |
| 3.8 | **Stage 6 — `evaluate_coach_enable_gates` (machinery)** | ✅ DONE (machinery, 2026-07-04); real cert run gated on human α | agent | 3.7 | Built `services/governance/coach_calibration.py`: frozen `CoachGateDecision` + `evaluate_coach_enable_gates`, **fully self-contained** confusion+rate helpers (`decisions.md` AP-6 decoupling), κ REUSES `iaa` (NaN→None), **no `meta/` import** (services↛meta). Binding floor TPR≥0.90/TNR≥0.95/κ≥0.75 (inclusive); augmenting precision/false-action/flip + per-axis κ + production-subset precision are **diagnostic-only, never gate**. Fail-closed: `REFUSE_PROVISIONAL` **proven on the real 3.7 artifact** (provisional=true short-circuits before any metric); ENABLE/REFUSE/undecidable proven on synthetic non-provisional fixtures. 17 L1 tests (red-first); `make check` green (5037 passed). **REMAINING (3.9):** the real ENABLE/REFUSE cert run needs the non-provisional human-labeled gold set (α≥0.80, populated test split). Never flips `COACH_LEAKAGE_GATE_ENABLED` (human Phase-5 step). |
| 3.9 | **Phase-3 exit: cert report → ENABLE verdict** | 🔴 REFUSE (2026-07-05) | human+agent | 3.8 | Human α gate CLEARED — `coach_goldset_v1.json` now `provisional=false`, 116-row test split (29 leak / 87 clean). Real cert (`gpt-4o`, `cache/coach_eval/coach_enable_cert.json`): **verdict=REFUSE — TPR 0.966 ✓, κ ✓, TNR 0.9186 ✗** (floor ≥0.95). Confusion TP28/FN1/**FP7**/TN79 (+1 abstain, `T-CLEAN-20`, dropped from the TNR denominator: 79/(79+7)=0.9186) — judge is NOT missing leaks, it **over-flags clean teaching** (7 FP among 87 clean; ≤4 allowed). Per this row's own rule below-floor = telemetry-only; Phase 5 STAYS gated. Replay harness hardened (`run_coach_calibration --dump-labels --per-call-timeout`; a hung provider socket had frozen the first pass ~48 min). → routes to 3.10. |
| 3.10 | **Rubric specificity revision → fresh re-cert** (ADR-0018) | 🟡 PLANNED (ADR **Accepted** 2026-07-06; spec+plan+tasks drafted, human gate NEXT) | human+agent | 3.9 | Open coding of the 3.9 FPs (`cache/open_coding/coach-phase39-tnr-fps/coded.jsonl`, all 7 captured — an 8th flagged row abstained on a provider timeout and was dropped) axial-collapses to ONE category **OVERFLAG-1** (mechanism-teaching / open probe / locus-pointing read as item-collapse; **0** gold-dispute, **0** incoherent-read). [ADR-0018](../adr/0018-subject-coach-rubric-specificity-revision.md) (**Accepted**) + [prose spec](coach-rubric-specificity-revision.spec.md): **prose-only** carve-out in `subject_coach_pedagogy_judge.j2` (first-class CLEAN test + count-surviving-options + name the 2 over-reads; no schema change; `rubric_version`→`coach_rubric_v2_specificity`). **Fresh-split + re-cert SDD:** [spec](coach-fresh-recert-split.spec.md) + [plan/tasks](coach-fresh-recert-split.plan.md) — 3 tracks (A prose ∥ B fresh-split ∥ C-pre model-pin) converging at a hard gate → **C1 glm-5.2 re-cert, ≥3 temp-0 replays, zero-flip**. Decisions (user, 2026-07-06, `decisions.md`): **in-session-authored** ~40–60-row fresh split (separate `coach_recert_split_v1.json`, 3.9 stays frozen); re-cert on **glm-5.2** (+ gpt-4o diagnostic anchor — comparability caveat noted); **"margin" = TNR≥0.95 held zero-flip across ≥3 replays**. **Exit: every replay TNR≥0.95 ∧ TPR≥0.90 ∧ κ≥0.75** on the fresh split. Stage-4 surfaced (a) a needed harness model-pin (Task C-pre) and (b) a **pre-existing G8 blocker** (`3ea6a90` removed `test_refuse_provisional_on_real_artifact`; needs a synthetic-artifact test or `# G8-OK:` waiver before `make check` is green). Routes to sdd-implement after the tasks human gate. **RESOLVED → CERTIFIED (2026-07-06, ADR-0019, commit `dcb5b56`):** the fresh re-cert landed on **glm-5.2 via the Fireworks host adapter** — 3× temp-0 replays, **TNR 1.0 / TPR 1.0, zero-flip** (the gpt-4o REFUSE was an availability/over-flag artifact of that model, not the rubric). `coach_goldset_v1.json` re-froze `provisional=false`; findings in `docs/IAA/coach/recert/coach_recert_findings.md`. **Phase 5 is UNGATED** and now BUILT — see the Phase-5 board below. |
...
| 5.3 | §12.7 drift baselines + CI golden regression wiring | ✅ BUILT (CI-safe replay gate) | 5.1 | Bundle `coach-regression-gate.{spec,plan,tasks}.md`. `meta/coach_regression_gate.py` recomputes the ADR-0019 floor (TNR/TPR/κ) from the 3 COMMITTED recorded runs (`recert_labels_fw_run{1,2,3}.jsonl`) by calling `evaluate_coach_enable_gates` per run (certified evaluator VERBATIM) + `flip_rate` for zero-flip; fails on floor breach / cross-run flip / malformed-or-mislabeled artifact. CI-safe (grades committed snapshots, NO live LLM). Always-on pytest gate (`tests/meta/test_coach_regression_gate.py`, in `make check` + CI `pytest tests/`) + operator CLI `scripts/coach_regression_gate.py` (exit 0/1/2, mirrors `eval_regression_gate.py`). Implementation caught a real spec gap — run3's `R-CLEAN-29` judge **abstention** (`judge_leak=null`/`confusion="abstain"`) is dropped from the denominator exactly as the cert did (FR-5b), not an error. `make check` green (5177). **Scope pin:** CI-safe replay only; the live-refresh half + 2σ drift-delta (`meta/drift.py detect_performance_drift`) stay LATER (5.4-adjacent, need live traffic). |
