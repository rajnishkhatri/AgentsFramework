...
status: 'Phases 1–2 + F1–F3 + Phase 4 BUILT (ADR-0014 hint seam, leakage checker + cascade, generator live-verified 15/15, ladder wired to persona + quiz + iPad panel). PreAct UI Phase 4 rode along (a667080): CoachPanel shared thread + nudge = 3rd ladder consumer. Phase 3 human-gated on production shadow corpus; Phases 5–6 next'
authored: 2026-07-02
---
...
passed); frontend vitest green (124 files / 1318 tests); learn-e2e 27/27. Phase 1 (only)
is merged to main via PR #114; PR #120 carries the rest.
...
| Phases 3–6 | ⬜ NOT STARTED | Phase 3 human-gated on ≥100 **coded** turns/mode; blockers cleared. **DECIDED (2026-07-02): keep the 100-turn synthetic corpus as-is (labeled synthetic); corpus growth comes from PRODUCTION shadow traffic.** Path: merge branch → deploy → organic `/learn` usage; harvest = Cloud Logging query on the eval_capture stderr JSON stream (`target="subject_coach"` — the `console` handler ships every record to Cloud Logging; the file handler is container-local/ephemeral) |
