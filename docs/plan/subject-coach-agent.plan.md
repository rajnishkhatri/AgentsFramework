...
| 3.8 | **Stage 6 — `evaluate_coach_enable_gates` + cert run** | ⬜ **BLOCKED on human double-label** | agent | 3.7 | Mirror `evaluate_section_2_8_gates`. Binding floor: TNR ≥ 0.95, TPR ≥ 0.90, κ ≥ 0.75 on frozen test split. **Entry gate:** the 3.7 artifact is `provisional=true` (α=null, test split empty) → the cert returns `REFUSE_PROVISIONAL` until the human double-label pass (α ≥ 0.80) produces a non-provisional `coach_goldset_v1` with a populated test split. |
