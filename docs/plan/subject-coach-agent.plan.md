...
| 3.7 | **Stage 5 — `coach_goldset_v1` assembly** | ⬜ **NEXT** | agent | 3.6 | Build `CoachGoldsetItem` + `assemble_coach_goldset.py` (mirror GoalJudge). 200–300 rows, leak-class oversample, 60/40 dev/test frozen+hashed. Double-label, **α ≥ 0.80 on `answer_leakage`**. |
