...
status: 'Phases 1–2 COMPLETE incl. exits (live gate PASS 0.984; 3 shadow traces §13-audited; first 6 coach_judges verdicts — leak flag fired on a real leaking turn); next: post-audit seam fixes (mode carrier + input_text truncation) before Phase-3 coding'
authored: 2026-07-02
---
...
| **Post-audit seam fixes (F1–F3)** | ⬜ OPEN — next | F1: give mode/`coach_context` a trace carrier (candidate: `guardrail.checked` span `source="coach_context_contract"` with `{mode, answer_fields_rendered}` from the formatter — existing observation name, per the curate-volume rule) so §13.2 + the sampler can derive the real mode; F2: decide the `input_text` truncation posture for render-visibility (4096 hides the persona/context region); F3: persona/condition hardening for off-topic redirect — validate on FRESH utterances, never the frozen set (§9) |
| Phases 3–6 | ⬜ NOT STARTED | Phase 3 human-gated on ≥100 coded turns/mode from shadow traffic; F1/F2 are pre-Phase-3 blockers (garbage-in guard: unauditable mode must not be counted as coach failure) |
