...
status: 'Phase 3 COMPLETE — 2026-07-06: 3.0–3.8 + corpus-expansion E1–E6 DONE; 3.9 cert exit CERTIFIED. First cert (gpt-4o) REFUSED (TNR 0.9186<0.95, 7 FP → over-flag); recert round (ADR-0018 rubric revision → ADR-0019 Fireworks re-host) CERTIFIED the answer-leakage judge on glm-5.2-fireworks: 3× temp-0 replays all ENABLE, TNR 1.0/TPR 1.0/κ pass, 0 FP, zero-flip (frozen coach_recert_split_v1.json, 47 rows). make check 5114 pass. COACH_LEAKAGE_GATE_ENABLED stays OFF (Phase-5 flip is separate).'
authored: 2026-07-03
---
...
| 3.9 Cert exit → ENABLE/REFUSE | ✅ **DONE — CERTIFIED** (ADR-0019) | Two live cert rounds, both no-LLM-in-CI (labels committed + replayed offline).<br>**Round 1 — gpt-4o on the 246-row set → REFUSE** (`coach_enable_cert.json`): TPR pass · **TNR 0.9186 fail (<0.95)** · κ pass; 7 FP (over-flag). Recorded, not relaxed.<br>**Round 2 — recert (ADR-0018 rubric revision → ADR-0019 Fireworks re-host).** Root cause: glm-5.2 on Z.ai stalled → broke the FR-9 temp-0 zero-flip; re-hosted behind the same `provider="direct"` port (no new dep, no schema change). 4-candidate screen picked **glm-5.2-fireworks** on merit (TNR 1.0/0 FP; DeepSeek-V4-Pro 6 FP, GPT-OSS-120B 10 FP, Kimi-K2.6 stalled). **3× temp-0 cert on the frozen `coach_recert_split_v1.json` (47 rows): all ENABLE, TNR 1.0 / TPR 1.0 / κ pass, precision 1.0, 0 FP, zero-flip** (runs 1/2/3 mechanically verified). commit `dcb5b56`; `make check` 5114 pass.<br>**Gate stays OFF:** `COACH_LEAKAGE_GATE_ENABLED` unchanged — the Phase-5 flip (shadow → enforce) is a separate human step. |
